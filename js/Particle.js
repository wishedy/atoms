$(document).ready(function(){
    var Particle = {
        ...
        startAnimate:function(){
          var t = this;
          var sumTime = 0;
          t.animateTimer = setInterval(function(){
              if(sumTime===t.Particles.length){
                  ...
                  if(t.sequence===4){
                      ...
                      var circle = new zrender.Image(
                          {
                              'style': { x: 0,y:0,width:600,height:270, image:newImg.src}
                          }
                      );
                      t.zr.add(circle);
                      circle.animateTo({
                          style: {
                              x:t.centerImg.x-150, y: t.centerImg.y-60,
                              width:800,
                              height:500
                          }
                  ...
                  switch (t.sequence) {
                      case 0:
                          for (var num = 0; num < t.Particles.length; num++) {
                              ...
                              item.rect.animateTo({
                                  style: {
                                      width: t.atomW - 6, height: t.atomH - 6,
                                      x: item.x, y: item.y
                                  }
                              }, 3000, 10, 'linear', function (e) {
                                  // done
                              ...
                              itemD.rect.animateTo(
                                  {
                                      style: {
                                          width:t.atomW*1.5,
                                          height:t.atomH*1.5,
                                          x:t.atomW*itemD.disx*.3, y: itemD.disy*t.atomH*.3
                                      }
                                  }, 3000, 10, 'linear', function () {
                                      // done
                                      console.log('执行3');
        ...
        getCenterDis:function(){
            ...
            var atom ={
                ...
                rect:new zrender.Image(
                    {
                        'style': { x: t.centerImg.x, y: t.centerImg.y,width:t.atomW - 6,height:t.atomW - 6, image:t.centerImg.imgSrc}
                    }
                )
            } ;
        ...
        createAtoms:function(x,y,z,w,h){
            ...
            return {
                ...
                locy: parseInt(Math.random() * t.panelH),
